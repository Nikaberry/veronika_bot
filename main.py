...
from country_game import country
...
with open('max_result_countries.txt', 'r') as f:
    max_country_score = int(f.read())
...
def show_rules(call):
    ...
    rules_text = ("Вот правила игры: "
                  ...
                  "России. Если вы играете в страны, то бот дополнительно проверяет"
...
def back_to_menu(call):
    global max_word_score, max_city_score, word_score, city_score, max_country_score
    ...
    if country_score > max_country_score:
        max_country_score = country_score
        with open("max_result_countries.txt", "w") as file:
            file.write(str(max_country_score))
    ...
    bot.send_message(call.message.chat.id, f"Максимальный результат в игре в города: {max_city_score}",
                     reply_markup=markup)
    bot.send_message(call.message.chat.id, f"Максимальный результат в игре в страны: {max_country_score}",
                     reply_markup=markup)
...
def choose_game(call):
    ...
    country_game_button = types.InlineKeyboardButton('Игра в страны', callback_data='countries_game')
    markup.add(cities_game_button, words_game_button, country_game_button)
    bot.send_message(call.message.chat.id, "У меня есть три игры: города России, игра в слова и игра в "
                                           "страны, что выберешь?",
                     reply_markup=markup)
...
@bot.callback_query_handler(func=lambda call: call.data == 'countries_game')
def start_countries_game(call):
    global current_game
    current_game = "countries"
    markup = types.InlineKeyboardMarkup(row_width=1)
    finish_button = types.InlineKeyboardButton("Завершить игру", callback_data='finish_game')
    markup.add(finish_button)
    bot.send_message(call.message.chat.id, "Давайте начнем игру в страны! Назовите страну на любую букву.",
                     reply_markup=markup)
...
def handle_message(message):
    global current_game, used_words, word_score, city_score, country_score
    ...
    if current_game == 'countries':
        ...
        if len(used_words) == 0:
            ...
            country_score += 1
            return
        elif word not in country:
            bot.send_message(message.chat.id, f"Такой страны нет, назови другую страну.",
                             reply_markup=markup)
        elif word in used_words:
            bot.send_message(message.chat.id, "Эта страна уже была использована. Пожалуйста, введите другую страну.",
                             reply_markup=markup)
        elif word[-1] in "ьъы" and word not in used_words:
            bot.send_message(message.chat.id, f"Отлично! Теперь назови страну, начинающуюся на букву "
                                              f"'{word[-2].upper()}'.",
                             reply_markup=markup)
            used_words.append(word)
            country_score += 1
